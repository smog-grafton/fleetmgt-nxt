    ...
    <SheetBody className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6">
      ...
      {Object.entries(schema.fields).map(([name, field]) => {
        ...
        return <div className="space-y-2" key={name}><Label htmlFor={`field-${name}`}>{field.label}{required && <span className="ms-1 text-destructive">*</span>}</Label>
          ...
          : field.type === 'file' ? <label className="flex cursor-pointer items-center gap-4 rounded-xl border border-dashed p-4 transition-colors hover:bg-muted/50"><span className="grid size-10 place-items-center rounded-lg bg-primary/10 text-primary"><FileUp /></span><span className="min-w-0"><span className="block text-sm font-medium">{files[name]?.name || 'Choose a file'}</span><span className="block text-xs text-muted-foreground">PDF, image or document up to the backend limit</span></span><Input id={`field-${name}`} type="file" className="sr-only" onChange={(event) => setFiles((current) => ({ ...current, [name]: event.target.files?.[0] || null }))} /></label>
